...
variable "vcn_name"         { default = "demoVCN" }
variable "vcn_dns"          { default = "demovcn" }
variable "vcn_ig_name"      { default = "demoIG" }
variable "vcn_route_name"   { default = "demoRouteTable" }

# Variables for public subnet
variable "pub_sec_name"     { default = "demoPubSecName" }
variable "pub_subnet1"      { default = "10.1.0.0/24" }
variable "pub_subnet1_name" { default = "pub_subnet1" }
variable "pub_subnet1_dns"  { default = "pubsub1" }

# Variables for Compute
variable "webHost1"        { default = "demoHostWeb01" }
variable "InstanceShape"   { default = "VM.Standard1.2" }
variable "publicIP"        { default = "true" }
variable "os_name"         { default = "Oracle Linux" }
variable "os_version"      { default = "7.8" }
...
resource "oci_core_route_table" "vcnRoute" {
    ...
    route_rules {destination = "0.0.0.0/0" network_entity_id = "${oci_core_internet_gateway.ig.id}"}
}
...
resource "oci_core_security_list" "vcnSec" {
    ...
    {tcp_options {"max" = 80 "min" = 80} protocol = "6" source = "0.0.0.0/0"},
    {tcp_options {"max" = 22 "min" = 22} protocol = "6" source = "0.0.0.0/0"},
    {protocol = "6" source = "${var.vcn_cidr}"}]
}
...
resource "oci_core_instance" "webhost01" {
    ...
    create_vnic_details { subnet_id = "${oci_core_subnet.pubSubnetAD1.id}" assign_public_ip = "${var.publicIP}" },
    metadata {ssh_authorized_keys = "${file(var.public_key_path)}" user_data = "${base64encode(var.user-data)}" }
    timeouts { create = "60m" }
}
...
data "oci_core_vnic" "webhost01Vnics" {
    vnic_id = "${lookup(data.oci_core_vnic_attachments.webhost01VnicsAttach.vnic_attachments[0],"vnic_id")}"
}

# Output
output "webhost01_public_ip" { value = "${data.oci_core_vnic.webhost01Vnics.public_ip_address}"}
